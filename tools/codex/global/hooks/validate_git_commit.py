...
AI_METADATA_PATTERN = re.compile(
    r"(?:^|\n)(?:co-authored-by:.*(?:codex|chatgpt|openai)|made[- ]with:|generated by:|ai[- ]generated:)|"
    r"(?:made with|generated by)\s+(?:codex|chatgpt|openai)",
    re.IGNORECASE,
)
...
def extract_messages(arguments: Sequence[str]) -> list[str]:
    ...
    if not messages:
        block("Codex git commits must use an explicit -m subject (and optional additional -m body arguments).")
    return messages
